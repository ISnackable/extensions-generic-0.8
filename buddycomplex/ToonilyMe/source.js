...
var source = (() => {
  ...
  var BuddyComplexParser = class {
    constructor() {
      this.parseViewMore = ($, languages = ["English"]) => {
        const mangas = [];
        const collectedIds = [];
    ...
    parseHomeSections($, sections, sectionCallback, languages = ["English"]) {
...
Please go to Settings > Sources > ${this.baseUrl} and press Cloudflare Bypass`);
  ...
  // src/ToonilyMe/ToonilyMeParser.ts
  var ToonilyMeParser = class extends BuddyComplexParser {
    constructor() {
      super(...arguments);
      this.parseViewMore = ($, languages = ["English", "Japanese"]) => {
        const mangas = [];
        const collectedIds = [];
        for (const manga of $("div.book-item", "div.list").toArray()) {
          const id = this.idCleaner($("a", manga).attr("href") ?? "");
    ...
    parseHomeSections($, sections, sectionCallback, languages = ["English", "Japanese"]) {
      for (const section of sections) {
        if (section.id == "hot_updates") {
          const HotUpdates = [];
          for (const manga of $(
  ...
  var ToonilyMe = class extends BuddyComplex {
    constructor() {
      ...
      this.parser = new ToonilyMeParser();
              ...
              App.createDUISection({
                ...
                rows: async () => [
                  App.createDUISelect({
                    ...
                    labelResolver: async (option) => ["English", "Japanese"].find(
                      (language) => language === option
                    ) ?? "Unknown",
                    value: App.createDUIBinding({
                      get: async () => await stateManager.retrieve("languages") ?? [
                        "English",
                        "Japanese"
                      ],
                      set: async (newValue) => {
      ...
      this.resetSettings = (stateManager) => {
    ...
    async getHomePageSections(sectionCallback) {
      const languages = await this.stateManager.retrieve(
        "languages"
      ) ?? ["English", "Japanese"];
