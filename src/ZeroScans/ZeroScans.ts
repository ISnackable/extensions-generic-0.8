        ...
        HomePageSectionsProviding
{
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        ...
        let numericId = id_html.substring(
            start_index + 19,
            id_html.indexOf(',name:')
        )
    ...
    async createChapterRequest(numericId: string, page: number): Promise<any> {
        const url = `${
            this.baseUrl
        }/swordflake/comic/${numericId}/chapters?sort=asc&page=${page.toString()}`
        const request = App.createRequest({
            url,
            method: 'GET',
        })
        const response =
            (await this.requestManager.schedule(request, this.RETRY)) ?? ''
